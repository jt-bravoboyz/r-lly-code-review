-- Fix: get_event_safe / list_events_safe were declared RETURNS SETOF events
-- but their SELECT lists used a different column order than the events table.
-- Postgres maps SETOF events rows POSITIONALLY, so start_time was landing in
-- the is_barhop slot and the function errored / returned no rows. EventDetail
-- then redirected to /events for any opened R@lly. Rebuild both functions
-- with columns in the EXACT positional order of the events table.
