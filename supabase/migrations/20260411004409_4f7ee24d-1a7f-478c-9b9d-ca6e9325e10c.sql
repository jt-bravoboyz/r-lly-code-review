...
-- Update handle_new_user to support founding_member metadata
...
BEGIN
  ...
  -- Check if name looks like gibberish (Apple-style random strings)
  _needs_name := false;
  ...
  _is_founding := false;
  _founder_num := NULL;
  IF (NEW.raw_user_meta_data->>'founding_member') = 'true' THEN
...
-- Create claim_founding_spot RPC for post-OAuth flow
