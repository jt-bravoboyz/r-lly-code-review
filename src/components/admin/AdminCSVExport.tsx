...
export function AdminCSVExport({ events, attendees, label = 'Export CSV' }: AdminCSVExportProps) {
  const handleExport = () => {
    const header = 'Event Name (Masked),City,Headcount,Total Attendees,% Safety Confirmed,Transit Provider Used';
    const rows = events.map((event, i) => {
      const ea = attendees.filter(a => a.event_id === event.id);
      const total = ea.length;
      const headcount = ea.filter(a => (a as any).status === 'attending').length;
      ...
      return `"R@lly #${i + 1}","${city}",${headcount},${total},${safetyPct}%,"${providers}"`;
    });
