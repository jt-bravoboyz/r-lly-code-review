  ...
  return (
    <div className="border border-white/10 bg-white/5 p-3 my-4 rounded-2xl overflow-hidden">
      ...
      <div className="space-y-3">
        {/* Map preview */}
        <div className="relative w-full h-[180px] rounded-xl overflow-hidden border border-white/10 bg-zinc-900">
          {mapUrl && (
            <img
              src={mapUrl}
              alt=""
        ...
        <div className="flex gap-2 mt-3">
          {[
            { Icon: Car, label: 'OFFER SEAT' },
            { Icon: Shield, label: 'BE DD' },
            { Icon: MapPin, label: 'PICKUP' },
          ].map(({ Icon, label }) => (
            <div
              key={label}
              className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg bg-white/5 border border-white/10 transition-all duration-500 ease-out ${
