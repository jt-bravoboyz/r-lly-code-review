...
export default function SplitCheckPreview() {
  ...
  return (
    <div className="border border-white/10 bg-white/5 p-3 my-4 rounded-2xl overflow-hidden">
      ...
      <div className="relative w-full h-[260px] rounded-xl bg-[#0a0a0a] border border-white/10 overflow-hidden">
        {/* Receipt */}
        <div
          className="absolute inset-3 top-3 z-10 flex flex-col transition-opacity duration-300"
          style={{ opacity: receiptVisible ? 1 : 0 }}
        >
          <div className="text-[9px] uppercase tracking-widest text-white/40 pb-2 mb-2 border-b border-dashed border-white/15">
          ...
          {ITEMS.map((item) => {
            ...
            return (
              <div
                key={item.key}
                className="flex items-center justify-between py-1.5 text-[11px] transition-opacity duration-200"
                style={{ opacity: itemRowVisible[item.key] ? 1 : 0 }}
              >
                <div className="flex items-center gap-2 min-w-0">
                  <div className="flex -space-x-1 w-[34px] justify-start">
                    {placed.map((p) => avatarCircle(p, 16))}
                  </div>
        ...
        <div
          className="absolute top-1 left-3 right-3 z-30 flex gap-1 transition-opacity duration-300"
        ...
        {tabVisible && (
          <div
            className="absolute bottom-3 right-3 z-30 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#F47A19] text-black text-[10px] font-bold tracking-wide shadow-[0_0_12px_rgba(244,122,25,0.5)] transition-all duration-[250ms]"
            style={{
        ...
        {dialogVisible && (
          <div
            className="absolute bottom-3 left-3 right-3 z-20 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15 p-3 flex gap-2 transition-all duration-300"
            style={{
          ...
          >
            <div
              className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-[#F47A19]/20 border border-[#F47A19]/40 text-[10px] font-bold text-[#F47A19] transition-shadow duration-200"
              style={{
            ...
            <div className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-white/5 border border-white/10 text-[10px] font-bold text-white/70">
              <Upload size={12} />
              UPLOAD
            </div>
          </div>
        ...
        {/* Final summary */}
        <div
          className="absolute bottom-3 left-3 right-3 z-20 rounded-xl bg-[#F47A19]/15 border border-[#F47A19]/40 p-2 flex items-center justify-around transition-all duration-500"
          style={{
        ...
        >
          {SUMMARY.map((s) => (
            <div key={s.key} className="flex flex-col items-center gap-0.5">
              ...
              <div className="text-[11px] font-bold text-[#F47A19]">{s.amount}</div>
            </div>
          ))}
        </div>
      </div>
