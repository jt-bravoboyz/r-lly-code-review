...
export function DDSetupDialog({
  ...
  useEffect(() => {
    const fetchAttendees = async () => {
      ...
      try {
        ...
        } = await supabase.from('event_attendees').select(`
            profile_id,
            profile:profiles!event_attendees_profile_id_fkey(id, display_name, avatar_url)
          `).eq('event_id', eventId).neq('profile_id', profile.id);
        if (attendeesError) throw attendeesError;

        // Get passengers who already have accepted rides for this event
        const {
          data: passengers
        } = await supabase.from('ride_passengers').select('passenger_id, rides!inner(event_id)').eq('rides.event_id', eventId).eq('status', 'accepted');
        const passengersWithRides = new Set(passengers?.map(p => p.passenger_id) || []);

        // Filter and mark attendees
        const eligibleAttendees = (attendees || []).map(a => ({
  ...
  const togglePriorityPerson = useCallback((profileId: string) => {
    setPriorityPeople(prev => prev.includes(profileId) ? prev.filter(id => id !== profileId) : [...prev, profileId]);
  }, []);
  ...
  const getEventChatId = async (eventId: string): Promise<string | null> => {
    const {
      data
    } = await supabase.from('chats').select('id').or(`event_id.eq.${eventId},linked_event_id.eq.${eventId}`).maybeSingle();
    return data?.id || null;
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (existingRide) {
        ...
        } = await supabase.from('rides').select('id').eq('event_id', eventId).eq('driver_id', profile.id).in('status', ['active', 'full', 'paused']).maybeSingle();
      ...
      if (priorityPeople.length > 0 && rideId) {
        ...
        const {
          error: offersError
        } = await supabase.from('ride_offers').upsert(offers, {
          onConflict: 'ride_id,to_profile_id'
        });
        if (offersError) {
      ...
      queryClient.invalidateQueries({
        queryKey: ['is-dd', eventId]
      });
      queryClient.invalidateQueries({
        queryKey: ['dd-ride', eventId]
      });
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="flex items-start gap-3">
                  <Checkbox id="liability" checked={acknowledgedLiability} onCheckedChange={checked => setAcknowledgedLiability(checked === true)} />
              ...
              <Button className="w-full mt-4" onClick={handleDisclaimerAccept} disabled={!acceptedTerms || !acknowledgedLiability}>
      ...
      <>
            ...
            <div className="space-y-5 py-4">
              ...
              {needsPickupSelection && <div className="space-y-2">
                  <Label className="flex items-center gap-2">
                    Pickup Location <span className="text-destructive">*</span>
                  </Label>
                  <LocationSearch value={pickupInputText} onChange={v => {
            ...
            }} placeholder="Search for pickup location..." showMapPreview={false} />
                  {pickupInputText && !pickupLocation && <p className="text-xs text-amber-600">Please select a location from the dropdown</p>}
                </div>}

              {/* Available Seats */}
              <div className="space-y-2">
                <Label htmlFor="seats">Available Seats</Label>
                <Input id="seats" type="number" min="1" max="8" value={availableSeats} onChange={e => setAvailableSeats(Math.max(1, Math.min(8, Number(e.target.value))))} />
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Select people to offer rides to first</p>
                
                {isLoadingAttendees ? <div className="flex items-center justify-center py-4">
                    <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
                  </div> : attendeesError ? <div className="text-sm text-destructive bg-destructive/10 rounded-lg p-3">
                    {attendeesError}
                  </div> : eventAttendees.length > 0 ? <ScrollArea className="h-40 rounded-lg border p-2">
                    <div className="space-y-1">
                      {eventAttendees.map(attendee => <div key={attendee.profile_id} onClick={() => togglePriorityPerson(attendee.profile_id)} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted cursor-pointer transition-colors">
                          <Checkbox checked={priorityPeople.includes(attendee.profile_id)} onCheckedChange={() => togglePriorityPerson(attendee.profile_id)} />
                ...
                {priorityExceedsSeats && <p className="text-xs text-destructive flex items-center gap-1">
              ...
              <div className="space-y-2">
                ...
                <Textarea id="notes" placeholder="e.g., Blue Honda Civic, can fit 4 comfortably..." value={notes} onChange={e => setNotes(e.target.value)} rows={2} />
              </div>
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 flex gap-3">
                ...
                <p className="text-xs text-muted-foreground">
                  {priorityPeople.length > 0 ? `${priorityPeople.length} people will get priority access to your ride for 15 minutes before it opens to everyone.` : 'Your ride will be visible to all event attendees.'}
                </p>
              </div>

              {/* Submit Button */}
              <Button className="w-full" onClick={handleComplete} disabled={isSubmitting || priorityExceedsSeats || needsPickupSelection && !pickupLocation}>
                {isSubmitting ? <>
