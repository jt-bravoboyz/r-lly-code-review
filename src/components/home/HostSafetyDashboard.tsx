...
import { CheckCircle2, Navigation, HelpCircle, XCircle, Car, PartyPopper, Clock } from 'lucide-react';
...
function SafetyStateBadge({
  ...
  switch (state) {
    case 'arrived_safely':
      return <Badge className="bg-green-100 text-green-700 border-green-300 text-[10px]">
          <CheckCircle2 className="h-2.5 w-2.5 mr-1" />
          {label}
        </Badge>;
    case 'participating':
      return <Badge className="bg-orange-100 text-orange-700 border-orange-300 text-[10px]">
          <Navigation className="h-2.5 w-2.5 mr-1" />
          {label}
        </Badge>;
    case 'not_participating':
      return <Badge variant="outline" className="text-muted-foreground text-[10px]">
          <XCircle className="h-2.5 w-2.5 mr-1" />
          {label}
        </Badge>;
    case 'undecided':
      return <Badge className="bg-amber-100 text-amber-700 border-amber-300 text-[10px]">
    ...
    case 'dd_pending':
      return <Badge className="bg-primary/20 text-primary border-primary/30 text-[10px]">
...
export function HostSafetyDashboard({
  ...
  useEffect(() => {
    ...
    if (previousSafetyComplete.current === false && safetyComplete === true) {
